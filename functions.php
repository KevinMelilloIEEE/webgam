<?php
  ...
  function emailLookup($email) {
    $org = shell_exec("/home/gamuser/bin/gam/gam info user $email | grep 'Google Org Unit Path:'");
    $org = str_replace("Google Org Unit Path: ","",$org);
    $vacation = shell_exec("/home/gamuser/bin/gam/gam user $email show vacation");
    $delegates = shell_exec("/home/gamuser/bin/gam/gam user $email show delegates | grep 'Delegate Email:'");
    return array ($org, $delegates, $vacation);
    }

  function groupLookup($group_email) {
    $managers = shell_exec("/home/gamuser/bin/gam/gam info group $group_email | grep 'manager:'");
    $managers = str_replace("manager: ","",$managers);
    $members = shell_exec("/home/gamuser/bin/gam/gam info group $group_email | grep 'member:'");
